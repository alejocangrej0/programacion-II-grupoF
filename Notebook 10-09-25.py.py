...
def cargar_desde_archivo_pacientes():
    try:
        ...
        open("pacinte.txt","w",encoding="utf-8").close()
...
def eliminarPacientes():
    ...
    if seleccionado:
        ...
        if messagebox.askyesno("Eliminar Paciente",f"estas segurode eliminar el paciente'{treeview.item(id_item,'values')[0]}'"):
            del pacientes_data[indice]
            guardar_e_archivo() #guardar los cambios en el archivo
            cargar_treeview()
            messagebox.showinfo("Eliminar Paciente","Se ha eliminado el paciente.")
    else:   
...
#Boton eliminar
btn_eliminar=tk.Button(btn_frame,text="Eliminar",command=eliminarPacientes)
...
cargar_desde_archivo_pacientes()    #cargar pacientes desde el archivo de la aplicacion
