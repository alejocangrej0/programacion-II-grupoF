...
def nuevoPaciente():
    ...
    ventanaRegistro.geometry("800x600")
    ...
    #Es una variable especial de Tkiter que se utiliza para enlazar widgets como radiobuttons
    ...
    cdDiabetes=tk.Checkbutton(ventanaRegistro,text="Diabetes", variable=diabetes,bg="lightblue")
    cdDiabetes.grid(row=5, column=1, sticky="w")


    cdHipertencion=tk.Checkbutton(ventanaRegistro,text="Hipertencion", variable=hipertension,bg="lightblue")
    cdHipertencion.grid(row=6, column=1, sticky="w")


    cdAsma=tk.Checkbutton(ventanaRegistro,text="Asma", variable=asma,bg="lightblue")
...
def nuevoDoctor():
    ventanaRegistro=tk.Toplevel(ventanaPrincipal) #ventana secundaria
    ventanaRegistro.title("Sistema de Registro de Doctores")
    ventanaRegistro.geometry("800x600")
    ventanaRegistro.configure(bg="lightblue")
    
    nombreLabel=tk.Label(ventanaRegistro,text="Nombre completo:",bg="lightblue")
    nombreLabel.grid(row=0, column=0, padx=10, pady=5, sticky="w")
    entryNombre=tk.Entry(ventanaRegistro)
    entryNombre.grid(row=0, column=1, padx=10, pady=5, sticky="we")
    entryNombre.configure()
    
    direccionLabel=tk.Label(ventanaRegistro,text="Direccion:",bg="lightblue")
    direccionLabel.grid(row=1, column=0, padx=10, pady=5, sticky="w",)
    entryDireccion=tk.Entry(ventanaRegistro)
    entryDireccion.grid(row=1, column=1, padx=10, pady=5, sticky="we")
    entryDireccion.configure()
   
    telefonoLabel=tk.Label(ventanaRegistro,text="Telefono:",bg="lightblue")
    telefonoLabel.grid(row=2, column=0, padx=10, pady=5, sticky="w",)
    entryTelefono=tk.Entry(ventanaRegistro)
    entryTelefono.grid(row=2, column=1, padx=10, pady=5, sticky="we")
    entryTelefono.configure()
    
    EspecialidadLabel=tk.Label(ventanaRegistro, text="Especialidad:",bg="lightblue")
    ...
    rbPediatria=tk.Radiobutton(ventanaRegistro, text="pediatria", variable=Espacialidad, value="pediatria",bg="lightblue")
    rbPediatria.grid(row=3, column=1, sticky="w")    
    
    rbCardiologia=tk.Radiobutton(ventanaRegistro, text="Cardiologia", variable=Espacialidad, value="Cardiologia",bg="lightblue")
    rbCardiologia.grid(row=4, column=1, sticky="w")
    
    rbNeurologia=tk.Radiobutton(ventanaRegistro, text="Neurologia", variable=Espacialidad, value="Neurologia",bg="lightblue")
    rbNeurologia.grid(row=5, column=1, sticky="w")
    
    turnoLabel=tk.Label(ventanaRegistro, text="Disponibilidad",bg="lightblue")
    turnoLabel.grid(row=6, column=0, padx=10, pady=5, sticky="w")
    mañana=tk.BooleanVar()
    tarde=tk.BooleanVar()
    noche=tk.BooleanVar()
   
    cdMañana=tk.Checkbutton(ventanaRegistro,text="Mañana", variable=mañana,bg="lightblue")
    cdMañana.grid(row=6, column=1, sticky="w")

    cdTarde=tk.Checkbutton(ventanaRegistro,text="Tarde", variable=tarde,bg="lightblue")
    cdTarde.grid(row=7, column=1, sticky="w")

    cdNoche=tk.Checkbutton(ventanaRegistro,text="Noche", variable=noche,bg="lightblue")
...
menuAyuda.add_command(label="Acerca de ...", command=lambda:messagebox.showinfo("Acerca de","Version 1.0-biomedicine system"))
